...
public class Pixel implements Comparable<Pixel> {
    ...
    Pixel(int[] c) {
    	...
    	for (int i = 0 ; i < 3 && i < c.length; i++) {
    		...
    		if (i == 0) {
    			this.r = Math.min(Math.max(0, c[i]),255);
    		} else if (i==1) {
    			this.g = Math.min(Math.max(0, c[i]),255);
    		} else if (i==2) { 
    			this.b = Math.min(Math.max(0, c[i]),255);
    		}
    	}
    ...
    public String toString() {
        return ("(" + Integer.toString(this.r) + ", " + Integer.toString(this.g) + 
    ...
    public boolean equals(Pixel px) {
        return (this.r == px.getRed() && this.g == px.getGreen() &&
        		this.b == px.getBlue());
    }
